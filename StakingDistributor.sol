...
contract Distributor is Policy {
    ...
    function distribute() external returns (bool) {
        if (nextEpochTime > 0 && nextEpochTime <= uint32(block.timestamp)) {
    ...
    function nextRewardAt(uint256 _rate) public view returns (uint256) {
        return IERC20(Frozen).totalSupply().mul(_rate).div(1000000);
    }
     ...
     */
    function setEpochLength(uint32 _epochLength) external onlyPolicy {
        epochLength = _epochLength;
    }
     ...
     */
    function setEpochStartTime(uint32 _epochTime) external onlyPolicy {
        if (nextEpochTime == 0) {
