...
contract ETHFrozenBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable Frozen; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints Frozen when receives principle
    ...
    struct Bond {
        uint256 payout; // Frozen remaining to be paid
    ...
    function deposit(
        ...
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 Frozen ( underflow protection )
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(Frozen).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(Frozen).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(Frozen).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(Frozen).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Frozen supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(Frozen).totalSupply();
    ...
    /**
     *  @notice calculate amount of Frozen available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Frozen) to the DAO
     *  @return bool
     */
    function recoverLostToken(address _token) external returns (bool) {
        require(_token != Frozen);
